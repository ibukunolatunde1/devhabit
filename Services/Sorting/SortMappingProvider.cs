...
public sealed class SortMappingProvider(IEnumerable<ISortMappingDefinition> sortMappingDefinitions)
{
    public SortMapping[] GetMappings<TSource, TDestination>()
    {
        ...
        {
            throw new InvalidOperationException($"No sort mapping definition found for source type {typeof(TSource).FullName} and destination type {typeof(TDestination).FullName}");
        }
