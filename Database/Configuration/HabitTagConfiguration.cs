...
public sealed class HabitTagConfiguration : IEntityTypeConfiguration<HabitTag>
{
    public void Configure(EntityTypeBuilder<HabitTag> builder)
    {
        ...
        builder.HasOne<Tag>().WithMany().HasForeignKey(ht => ht.TagId); //.OnDelete(DeleteBehavior.Cascade);
        builder.HasOne<Habit>().WithMany(ht => ht.HabitTags).HasForeignKey(ht => ht.HabitId);
    }
}
